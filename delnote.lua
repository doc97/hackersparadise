...
function program:onEnter()
    ...
    elseif tonumber(self.args[1]) < 1 or tonumber(self.args[1]) > #Systems[Terminal.rootIp].notes then
        Terminal:endProg(-1, "NO NOTE WITH INDEX: " .. self.args[1])
    else
        table.remove(Systems[Terminal.rootIp].notes, tonumber(self.args[1]))
